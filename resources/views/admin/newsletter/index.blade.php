<x-admin-layout header="Kelola Newsletter">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-xl font-semibold">Daftar Subscriber Newsletter</h2>
                        <a href="{{ route('admin.newsletter.export') }}" class="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">
                        ...
                        <table class="min-w-full bg-white border border-gray-200">
                            <thead>
                                <tr>
                                    ...
                                    <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Tanggal Daftar</th>
                                ...
                                @forelse ($subscribers as $subscriber)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                            <div class="text-sm leading-5 text-gray-900">{{ $subscriber->id }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                        ...
                                        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                            <div class="text-sm leading-5 text-gray-900">{{ $subscriber->created_at->format('d-m-Y H:i:s') }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 font-medium">
                                            <form action="{{ route('admin.newsletter.destroy', $subscriber->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus email ini dari daftar newsletter?')">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-center">
                                            <div class="text-sm leading-5 text-gray-900">Belum ada subscriber newsletter.</div>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
