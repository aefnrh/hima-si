<x-admin-layout header="Edit Kabinet">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('admin.kabinet.update', $kabinet) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="mission" id="mission" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('mission', $kabinet->mission) }}</textarea>
                            @error('mission')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            @if ($kabinet->image)
                                <div class="mt-2 mb-2">
                                    <img src="{{ asset('images/' . $kabinet->image) }}" alt="{{ $kabinet->name }}" class="h-32 w-32 object-cover rounded">
                                </div>
                            @endif
