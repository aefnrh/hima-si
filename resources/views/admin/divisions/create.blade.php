<x-admin-layout header="Tambah Divisi">
                    ...
                    <form action="{{ route('admin.divisions.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="kabinet_id" id="kabinet_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                @foreach($kabinets as $kabinet)
                                    <option value="{{ $kabinet->id }}" {{ (old('kabinet_id') == $kabinet->id || (isset($selectedKabinet) && $selectedKabinet->id == $kabinet->id)) ? 'selected' : '' }}>{{ $kabinet->name }}</option>
                                @endforeach
                            </select>
                            @error('kabinet_id')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        ...
                        <div class="flex items-center justify-between mt-6">
                            @if(isset($selectedKabinet))
                                <a href="{{ route('admin.kabinet.show', $selectedKabinet) }}" class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">Kembali ke Kabinet</a>
                            @else
